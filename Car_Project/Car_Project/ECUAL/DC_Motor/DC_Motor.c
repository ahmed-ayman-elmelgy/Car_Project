...
MOTOR_ERROR_state 		MOTOR_Start (Motor_t   num ,STR_PWM_config_t * configurations)
{
	...
	else
	{
		if(Motor_State [num] != Motor_On)
		{
			...
			/* 3. start the motor! */
			//DIO_writePinValue(Arr_ST_Mtr_cfg[num][MOTOR_EN].PORT, Arr_ST_Mtr_cfg[num][MOTOR_EN].num_pin, DIO_u8_HIGH);

			/* 4. store the state for the next restart */
			Motor_State[num] = Motor_On;
		}
		else
		{
			return_value = MOTOR_WAS_ON;
		}
